 ...
 * @date Nov 13, 2020
 */
public class MyInvestListResult {
	...
	@Schema(title = "产品展示状态", required = true)
		...
		if(status==null) {
			if(getEnd()) {
				...
				if(startDate.getTime()<currentTime) {
					if(getTotalAmount().compareTo(getRaiseTotalAmount())>=0) {
						status=ContractProductStatusEnum.RAISE_FULL;
					}else {
						status=ContractProductStatusEnum.RAISE_PROCESSING;
					}
					if(getPartDate()!=null) {
						Date startInvestDate = TimeU.getDate(getPartDate(), 1);
						Date endInterestDate=TimeU.getDate(startInvestDate, getInvestDay());
						Date investEndDate=TimeU.getDate(endInterestDate, getRevertDay());
						...
						}else if(endInterestDate.getTime()<currentTime&&currentTime<investEndDate.getTime()) {
							status=ContractProductStatusEnum.SETTLEMENT_IN_PROGRESS;
						}else if(investEndDate.getTime()<currentTime) {
							status=ContractProductStatusEnum.END;
						}else {
